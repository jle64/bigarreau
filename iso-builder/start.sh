...
TARGET_IMAGE=${1:-$SQUASHFS_CTR_IMG}
OUT_DIR=${2:-./out}
...
podman-chroot "sed -i '1i spawn-sh-at-startup \"bootc-installer\"' /usr/share/tartaria/cherries/dot_config/niri/config.kdl || true"
...
podman-chroot 'runuser -u builder -- bash -c "cd /buildhome/bootc-installer && meson setup build --prefix=/usr --reconfigure && ninja -C build"'
podman-chroot 'cd /buildhome/bootc-installer && env DESTDIR=/ ninja -C build install || true'

# clone fisherman and install it to /usr/local/bin/fisherman
podman-chroot 'runuser -u builder -- bash -c "git clone --quiet https://github.com/projectbluefin/fisherman /buildhome/fisherman"'
podman-chroot 'runuser -u builder -- bash -c "cd /buildhome/fisherman/fisherman && GOCACHE=/buildhome/gocache GOPATH=/buildhome/gopath GOPROXY=off go build -o /buildhome/fisherman/fisherman-bin ./cmd/fisherman"'
podman-chroot 'install -Dm755 /buildhome/fisherman/fisherman-bin /usr/local/bin/fisherman'

# cleanup
podman-chroot 'userdel builder && rm -rf /buildhome /etc/sudoers.d'
podman-chroot 'pacman -Rns --noconfirm ninja meson blueprint-compiler mutter go'

# remove some pkgs to save space on ISO
podman-chroot 'for pkg in mkosi flatpak cups cups-browsed samba smbclient tuned ddcutil gpu-screen-recorder ttf-arphic-uming ttf-baekmuk wqy-microhei ttf-croscore ttf-droid gnu-free-fonts powertop libva-intel-driver bazaar; do pacman -Rns --noconfirm $pkg || true; done'

# create bootc-installer conf dir
podman-chroot 'mkdir -p /etc/bootc-installer'

# copy distro logo to icon dir
podman-chroot 'cp /usr/share/pixmaps/tartaria-text-logo.svg /usr/share/icons/default-icons-grey-dark/apps/scalable/distributor-logo-tartaria.svg || true'
...
podman-chroot "sed -i 's|ghcr.io/tartaria-dev/tartaria:TAG|${TARGET_IMAGE}|g' /etc/bootc-installer/recipe.json"
...
podman-chroot 'systemctl --global disable udiskie.service || true'
...
# Do not remove the base image if it's the target image we want to include!
...
# pull image to ISO
podman save --format oci-archive -o $_SCRIPTDIR/target-image.tar ${TARGET_IMAGE}
podman-chroot "TMPDIR=/var/tmp podman load -i /app/target-image.tar"
rm -f $_SCRIPTDIR/target-image.tar

github-step-end

github-step "Create Live User"

## Create liveuser & its home directory
...
podman-chroot "pacman -Sy --needed --noconfirm dracut"
