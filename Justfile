...
build-containerfile $image_name=image_name:
    sudo podman build --skip-unused-stages=false --volume "$PWD":/src -f Containerfile -t "localhost/{{image_name}}:{{image_tag}}" .

build-iso $image_name=image_name $image_tag=image_tag: (build-containerfile image_name)
    #!/usr/bin/env bash
    set -ouex pipefail
    sudo env "PATH=$PATH" ./iso-builder/start.sh "localhost/{{image_name}}:{{image_tag}}" ./out bigarreau
